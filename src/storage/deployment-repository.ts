...
export class DeploymentRepository {
  ...
  public getLatestDeployment(projectName: string): DeploymentRecord | null {
    ...
    const row = db
      .prepare(
        `SELECT id FROM deployments WHERE project_name = ? ORDER BY created_at DESC, id DESC LIMIT 1`,
      )
  ...
  public getDeploymentsByProject(projectName: string, limit = 20): DeploymentRecord[] {
    ...
    const rows = db
      .prepare(
        `SELECT id FROM deployments WHERE project_name = ? ORDER BY created_at DESC, id DESC LIMIT ?`,
      )
